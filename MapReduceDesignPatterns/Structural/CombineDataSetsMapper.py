    ...
    for line in reader:
        if len(line) == 5:
            if userHeader:
                ...
                generateUserStr(line)
        else:
            if postHeader:
                postHeader = False
            else:               
                generatePostStr(line)            

def generateUserStr(line):
    userId, reputation, gold, silver, bronze = line
    print "{0}\t{1}\t{2}\t{3}\t{4}\t{5}".format(userId, "user", reputation, gold, silver, bronze) 
    
def generatePostStr(line):
    nodeId, title, tag, userId, body, nodeType, parentId, absParentId, addedAt, score = line[0 : 10]
    print "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}".format(userId, "post", nodeId, title, tag, nodeType, parentId, absParentId, addedAt, score)     
